...
4. **On settlement,** payout repays any outstanding borrow first; surplus settles to wallet USDC balance. Debt-backed liquid credit is not externally withdrawable; only the net liquid amount above outstanding debt can leave `AgentAccountCore`. Agent can then optionally swap-and-stake into DOT-denominated yield strategies at v1.x (see revenue model below).

Borrow-to-stake is the durable model, not a v2 item. The native launch borrow facility exists in the contract suite already and is intentionally conservative: flat cap, over-collateralized, claim-stake-oriented, no liquidation, debt-gated withdrawals, and debt-first job settlement. Reputation-weighted caps and Hydration-backed borrow stay deferred.
...
- **Async XCM lane: dispatch and observer gap.** The untrusted raw-byte input surface is closed: HTTP accepts intent only and the backend assembler appends `SetTopic(requestId)` before calling `XcmWrapper`; the wrapper now verifies that `SetTopic` is the final declared SCALE/XCM instruction, not trailing bytes outside the instruction vector. `XcmWrapper.queueRequest` now calls the Hub XCM precompile `send(destination, message)` and reverts the whole queue transaction if that dispatch call fails. Remaining risk is native observer proof: until real staging evidence can correlate dispatched XCM outcomes back to Averray request IDs, async treasury endpoints stay admin-gated/staging-only.
...
- **Native borrow balance-sheet safety.** The launch contract path now blocks debt-backed liquid credit from external withdrawal and routes successful job payouts through debt repayment before surplus liquid credit. This is required because native borrow is balance-sheet credit, not a token transfer from an external lender reserve. Keep this invariant covered in contract tests whenever payout, withdrawal, or borrow semantics change.
...
- **XCM precompile dispatch.** The server-controlled assembler is shipped in `mcp-server/src/blockchain/xcm-message-builder.js`: HTTP rejects caller-supplied raw `destination`/`message`/`nonce`, backend assigns nonce, mirrors `previewRequestId(context)`, assembles XCM v5 bytes from strategy intent, appends `SetTopic(requestId)` as the last instruction, and submits assembled bytes to `XcmWrapper`. `XcmWrapper.queueRequest` now rejects zero `maxWeight.refTime`, rejects non-v5/empty-instruction envelopes, strict-decodes the supported v5 instruction subset so `SetTopic(requestId)` must be the final declared SCALE instruction rather than loose trailing bytes, and calls the Hub XCM precompile `send(destination, message)` before emitting the queued/payload/dispatched events. The backend asks `XcmWrapper.weighMessage` when the assembler only has a zero placeholder, preserves `uint64` weight components as exact values before submitting them, and preserves observer/finalize `uint256` settlement amounts as exact base-unit integers. Remaining work before vDOT mainnet is staging evidence for real precompile fees, XCM failure semantics, and native observer correlation on the chosen vDOT message shape.
...
- **Async strategy zero-share settlement guard.** `XcmVdotAdapter`/strategy settlement must not treat `status=Success` with `settledShares == 0` as a normal deposit. Either require nonzero shares for successful deposits or record an explicit loss/failure state with operator evidence. Without this guard, the account layer can book settled assets while the user receives zero strategy shares.
- **XCM weight proof requirement.** `XcmWrapper.weighMessage` currently tolerates precompile/weight failures by returning zero. Before live dispatch, queue/dispatch should require nonzero weight proof or an explicit staging-only bypass so operators cannot accidentally send unpriced XCM.
...
- **Liquidation mechanics for borrow facility.** Current `BORROW_CAP = 25 USDC` flat per account; no liquidation. Conservative `MIN_COLLATERAL_RATIO_BPS = 20000` (200%) plus debt-gated withdrawals and debt-first settlement hold the line until liquidation ships. v2 work.
...
- [x] `XcmWrapper.queueRequest` rejects zero ref-time weight and malformed non-v5 XCM envelopes before persisting requests
- [x] `XcmWrapper.queueRequest` strictly decodes the SCALE instruction vector and verifies the final instruction is `SetTopic(requestId)`, not only a matching byte suffix
- [x] `XcmWrapper.queueRequest` dispatches through the Hub XCM precompile rather than only recording an intent
...
2. **Borrow launch posture clarified:** v1 native borrow is flat-cap, over-collateralized, claim-stake-oriented, debt-gated, and debt-first on settlement. Reputation-weighted caps, liquidation, and Hydration-backed borrowing remain deferred.
3. **XCM hardening gaps narrowed:** `XcmWrapper` now rejects `SetTopic` bytes that sit outside the declared XCM instruction vector and requires `SetTopic(requestId)` to be the final declared instruction for the supported v5 message subset. Backend XCM weight handling preserves exact `uint64` values, and observer/finalize settlement handling preserves exact `uint256` base-unit amounts instead of coercing through JavaScript `Number`. `XcmWrapper.queueRequest` now calls the Hub XCM precompile `send(destination, message)` and reverts on precompile dispatch failure; native observer proof still gates production vDOT/yield claims.
...
3. **Async XCM status clarified:** the assembler, SetTopic validation, and wrapper precompile `send` call are shipped, but native observer proof remains an open launch gate for any production vDOT/yield claim.
