...
- **Async XCM lane: dispatch and observer gap.** The untrusted raw-byte input surface is closed: HTTP accepts intent only and the backend assembler appends `SetTopic(requestId)` before calling `XcmWrapper`; the wrapper now verifies that `SetTopic` is the final declared SCALE/XCM instruction, not trailing bytes outside the instruction vector. Remaining risk is that `XcmWrapper.queueRequest` is still a durable intent ledger, not a live call to the Hub XCM precompile, and the native observer proof is not yet production truth. Until precompile dispatch and observer evidence are captured, async treasury endpoints stay admin-gated/staging-only.
...
- **XCM precompile dispatch.** The server-controlled assembler is shipped in `mcp-server/src/blockchain/xcm-message-builder.js`: HTTP rejects caller-supplied raw `destination`/`message`/`nonce`, backend assigns nonce, mirrors `previewRequestId(context)`, assembles XCM v5 bytes from strategy intent, appends `SetTopic(requestId)` as the last instruction, and submits assembled bytes to `XcmWrapper`. `XcmWrapper.queueRequest` now rejects zero `maxWeight.refTime`, rejects non-v5/empty-instruction envelopes, and strict-decodes the supported v5 instruction subset so `SetTopic(requestId)` must be the final declared SCALE instruction rather than loose trailing bytes. The backend asks `XcmWrapper.weighMessage` when the assembler only has a zero placeholder and preserves `uint64` weight components as exact values before submitting them. Remaining work before vDOT mainnet is to turn `XcmWrapper.queueRequest` from intent-ledger storage into real Hub XCM precompile dispatch (`weighMessage` + `execute`/`send` as applicable), with failure semantics and fee/weight proof captured in staging.
...
- [ ] `XcmWrapper.queueRequest` strictly decodes the SCALE instruction vector and verifies the final instruction is `SetTopic(requestId)`, not only a matching byte suffix
...
3. **XCM hardening gaps narrowed:** `XcmWrapper` now rejects `SetTopic` bytes that sit outside the declared XCM instruction vector and requires `SetTopic(requestId)` to be the final declared instruction for the supported v5 message subset. Backend XCM weight handling preserves exact `uint64` values instead of coercing through JavaScript `Number`. Live XCM dispatch still needs precompile execution/failure semantics.
