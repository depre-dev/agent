...
import type { DisputeOrigin, DisputeSeverity, DisputeState } from "./types";

export interface DisputesFilter {
  state: DisputeState | "all";
  severity: DisputeSeverity | "all";
  origin: DisputeOrigin | "all";
  q: string;
}

const STATES: (DisputeState | "all")[] = [
  "all",
  "open",
  "awaiting-evidence",
  "under-review",
...
const SEVERITIES: (DisputeSeverity | "all")[] = [
  "all",
  "advisory",
  "gating",
  "hard-stop",
];
const ORIGINS: (DisputeOrigin | "all")[] = [
  "all",
  "signature",
  "schema",
  "co-sign-missing",
...
export function DisputesFilterRail({
  filter,
  onChange,
}: {
  filter: DisputesFilter;
  onChange: (next: DisputesFilter) => void;
}) {
  ...
  const set = <K extends keyof DisputesFilter>(k: K, v: DisputesFilter[K]) =>
    onChange({ ...filter, [k]: v });
  ...
  return (
    <div className="flex flex-col gap-2.5 rounded-[10px] border border-[var(--avy-line)] bg-[var(--avy-paper)] p-3 shadow-[var(--shadow-card)] backdrop-blur-[8px]">
      ...
      <Row label="Origin">
        {ORIGINS.map((o) => (
          <Chip key={o} on={filter.origin === o} onClick={() => set("origin", o)}>
            {o === "co-sign-missing"
              ? "co-sign missing"
              : o === "policy-violation"
                ? "policy violation"
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Filter by run, worker, verifier, receipt…"
