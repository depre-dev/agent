...
| `JWT_PUBLIC_KEY_PEM`    | `op://prod-backend/aws-jwt-signer-testnet/public-key-pem`   | ✅ yes            | deployer       | PEM-wrapped SPKI of the JWT KMS public key. Cached locally for verify — no per-request KMS call (`docs/PHASE_4B_KMS_JWT_PLAN.md` §4 option A). Drift-checked at boot (PR 4b.4+) and by `scripts/ops/verify-jwt-kms-signer.mjs` against `kms:GetPublicKey` for `AWS_JWT_KEY_ID`. |
